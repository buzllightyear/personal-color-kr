/**
 * Render test for the post-payment Guide route.
 *
 * Verifies that `app/(post-payment)/guide.tsx`:
...
// Stub `react-native` primitives used by the guide screen with strings,
...
describe('app/(post-payment)/guide route', () => {
  it('default-exports a React component (function)', () => {
    expect(typeof GuideScreen).toBe('function');
  });
  ...
  it('produces a renderable element tree via createElement', () => {
    const element = createElement(GuideScreen);
    expect(isValidElement(element)).toBe(true);
    expect(element.type).toBe(GuideScreen);
  });
});
