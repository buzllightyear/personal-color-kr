...
- **Session archive.** Dev-session deliberation (options weighed, why, open threads) is distilled into `docs/sessions/YYYY-MM-DD-<topic>.md` — **append-only, dated, NOT current-truth** (records what was thought on a date; cite the SSoT layers for current state). It complements, never replaces, STRATEGY/CLAUDE/ADR/memory. Because it's append-only+dated it does not drift. **Trigger (agent behavior):** the user signals before closing a session ("archive this") and the agent distills the entry; the agent ALSO flags an archive candidate in-turn the moment a decision or an OPEN thread closes — no fixed cadence, trivial sessions get no entry. See `docs/sessions/README.md`.
- **Operator protocol (agent behavior).** The operator is a non-developer; verification happens at the **contract layer**, not the code layer. Three standing rules for every substantive work report: **(1) Omission disclosure** — state explicitly what was NOT done, what was assumed without the operator's judgment, and what remains unverified; silence about undone work is the biggest verification leak. **(2) Invariant check** — `docs/INVARIANTS.md` is the operator-language contract list; if a change even *touches* an invariant, say so in the report ("touches invariant N — no violation, because …"), never pass silently. Adding/removing invariants is an operator decision only. **(3) Cross-model review for high-stakes changes** — auth/security, payments, PII/image storage, DB migrations or data deletion, and anything touching an invariant gets a heterogeneous-model review (codex-reviewer agent) before merge; the operator cannot judge these at code level, so single-model self-review is insufficient. Judgment slots (direction, product decisions, invariant changes) are never automated or absorbed by the agent — surface them as explicit questions.
