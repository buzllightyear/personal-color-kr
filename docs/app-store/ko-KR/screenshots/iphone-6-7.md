# 스크린샷 카피 사양 — iPhone 6.7형 (`1290 x 2796`) · ko-KR

> iOS App Store Connect의 **스크린샷(미리보기)** 슬롯 중 **6.7형(필수)** 사이즈 클래스용 한국어 카피 사양이에요.
...
> 오버레이 카피는 6.5형(`screenshots/iphone-6-5.md`)과 **동일**하게 사용해요. 해상도/안전 영역만 사이즈 클래스별로 달라요.
...
## 마스터 카피 표 — 6.7형 (`1290 x 2796`) · 6장 · 6열
...
5. 안전 영역: 상단 상태바·하단 홈 인디케이터를 오버레이 텍스트가 가리지 않도록 여백을 확보해요.
6. 데모/캡처용 셀카는 권리 확보된 이미지만 사용해요.

### TODO (캡처 전 사람이 처리 — true 미지수만)

- [ ] **TODO(human)**: 6.7형 6장 캡처 + 오버레이 합성 → PNG 6장 저장 (iPhone 15 Pro Max 시뮬레이터)
...
- [x] 해상도 `1290 x 2796` (6.7형 필수 사이즈 클래스) 명시
- [x] 내러티브 아크 퍼널 순서(`FUNNEL_SCREENS`) 고정
- [x] 모든 오버레이 해요체, 임의 창작 없이 코드베이스 유도
