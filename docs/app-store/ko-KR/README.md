...
- [ ] **3. Apple Developer 설정** — `appleTeamId`·`bundleIdentifier` 확정, App Store Connect 앱 레코드 생성, 인증서/프로비저닝 완료했어요. (담당/일자: ___)
  - [ ] **3-1. Apple Team ID** — `eas.json` 의 `TODO_APPLE_TEAM_ID` 를 실제 Apple Developer Team ID 로 교체했어요. (담당/일자: ___)
  - [ ] **3-2. ASC API Key ID** — App Store Connect → 사용자 및 액세스 → 통합 → App Store Connect API 에서 발급한 Key ID 로 `eas.json` 의 `TODO_ASC_API_KEY_ID` 를 교체했어요. (담당/일자: ___)
  - [ ] **3-3. ASC API Key Issuer ID** — 동일 화면 상단의 Issuer ID 로 `eas.json` 의 `TODO_ASC_API_KEY_ISSUER_ID` 를 교체했어요. (담당/일자: ___)
  - [ ] **3-4. ASC API Key `.p8` 파일** — 발급한 `.p8` 키 파일을 `apps/mobile/credentials/asc-api-key.p8` 경로에 배치했어요. (이 디렉터리는 `.gitignore` 로 커밋 제외 — 발급/배치 절차는 `apps/mobile/credentials/README.md`, 업로드 리허설은 `docs/testflight-dry-run.md` 참고) (담당/일자: ___)
