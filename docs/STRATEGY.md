...
- **검증 대기 (빌드 전/병행 게이트):** ① **바이럴 k-factor를 *계정-무관*으로 측정** — 소액 페이드 cold 코호트로 "낯선 사람끼리 k>1로 퍼지나"(= 진짜 바이럴 vs *내-팬이-나를-좋아함*). **벤처 사활이 여기 하나로 수렴.** *단 페이드는 k를 *재는* 도구지 *만드는* 게 아님(LTV<CAC면 성장채널 불가).* ② **craft 퀄** — 진짜 공유할 사진인가(`fal_eval`) + 의상 충실도 디스크리미네이터 없음(현 eval 얼굴만). ③ **early-sense hit-rate**(드롭당 반응). ④ 퍼스널컬러 개인화 신호 깊이. (15만 활성도·전이비율 = *부스터* 검증이지 사활 아님 — §2 강등.)
...
## 7. 다음 논의 — 피봇 ripple (퍼널 · Learn · 아이템 소싱)

`[open — 다음 세션 시작점. 2026-06-27 스코핑]`

피봇(콘텐츠/바이럴 VTON)이 건드리는 미해결 + 순서:

1. **코어 루프/퍼널 재설계 (먼저).** 기존 *결제 퍼널*(진단 hook → 12단계 Glam Up 결제) → **코어 루프**(획득 → 첫 VTON 사진[activation/aha] → **공유**[=k의 바이럴 액션] → 재방문). 목적이 "결제 전환" → "활성화+공유+리텐션".
   - 기존 빌드 중 *뭐 살리고 뭐 자르나*: 진단 hook(→ "너한테 뭐가 맞나" 개인화로 재결합?), 12단계 결제 퍼널(보류/폐기?), 레시피=스타일(→ 입어볼 트렌드 *아이템*으로 재용도?).
   - **아이템 소싱:** "사용자가 특정 아이템 골라 입어봄"의 아이템이 어디서 오나 — 업로드 / 큐레이션 카탈로그(= (a) 트렌드-큐레이션의 *제품 형태*). (a)를 제품에 박는 자리.
2. **day-1 Learn 계측 (그 위에).** 공유/귀속 이벤트(k 측정 — §6 게이트①, §5 D0/D1) + 행동(b 씨앗). *퍼널이 정해져야 이벤트 설계 가능* → 1 다음.
3. **craft + 아이템 소싱 병행** — 루프가 굴러가려면 둘 다.

**순서: ① 퍼널/코어 루프 → ② Learn 계측.** (Learn을 먼저 하면 허공에 이벤트 설계하는 꼴.)
